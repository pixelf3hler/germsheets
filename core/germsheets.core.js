...
(function(window, document, germSheets, undefined) {
   ...
   */
   ...
   function cloneFunction(fn) {
      var fnArr, fnBody, fnStr = fn.toString()
      
      if(-1 !== fnStr.indexOf("[native code]")) return fn
      
      // extract argument identifiers
      fnStr.replace(/\([^\)]*\)/i, function(m) {
         fnArr = "()" === m ? [] : m.substring(1, m.length-1).split(/\u0020*,\u0020*/)
         return m
      })
      
      // extract function body
      fnBody = fnStr.substring(fnStr.indexOf("{")+1, fnStr.lastIndexOf("}")).trim()
   ...
   function clone(obj) {
      if(null === obj || !(obj instanceof Object)) return obj
      var p, r, CloneConstructor = obj.constructor
      
      switch(CloneConstructor) {
         case RegExp:
            r = new CloneConstructor(obj.source, "g".substr(0, Number(obj.global)) + "i".substr(0, Number(obj.ignoreCase)) + "m".substr(0, Number(obj.multiline)))
         break
         
         case Date:
            r = new CloneConstructor(obj.getTime())
         break
         
         case Array:
         case Object:
            r = new CloneConstructor()
         break
         
         case Function:
            //var s = obj.toString()
            return cloneFunction(obj) //(-1 !== s.indexOf('[native code]')) ? obj : eval('(function() { return ' + s + ' })()')
